...
function MiniMap:new(dv, original_v_scrollbar)
  MiniMap.super.new(self, { direction = "v", alignment = "s",
...
-- Shift content origin rightward so it starts after the minimap.
-- Affects get_line_screen_position, resolve_screen_position, cursor scrolling, etc.
local old_docview_get_content_offset = DocView.get_content_offset
function DocView:get_content_offset()
  local x, y = old_docview_get_content_offset(self)
  if self.v_scrollbar and self.v_scrollbar:is(MiniMap)
    and self.v_scrollbar:is_minimap_enabled() then
    x = x + self.v_scrollbar.expanded_size
  end
  return x, y
end


-- DocView:draw() passes self.position.x directly to draw_line_gutter,
-- so we intercept here to add the minimap offset to the gutter position.
local old_docview_draw_line_gutter = DocView.draw_line_gutter
function DocView:draw_line_gutter(line, x, y, width)
  if self.v_scrollbar and self.v_scrollbar:is(MiniMap)
    and self.v_scrollbar:is_minimap_enabled() then
    x = x + self.v_scrollbar.expanded_size
  end
  return old_docview_draw_line_gutter(self, line, x, y, width)
end
